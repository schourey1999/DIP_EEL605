...
parser.add_argument('--n_epochs', type=int, default=200, help='number of epochs of training')  # Increased for smaller dataset
parser.add_argument('--batchSize', type=int, default=4, help='size of the batches')  # Reduced for 456 images
parser.add_argument('--lr', type=float, default=0.0002, help='initial learning rate')
parser.add_argument('--decay_epoch', type=int, default=30, help='epoch to start linearly decaying the learning rate to 0')  # Start decay earlier
parser.add_argument('--size', type=int, default=400, help='size of the data crop (squared assumed)')
parser.add_argument('--cuda', action='store_true', help='use GPU computation')
parser.add_argument('--n_cpu', type=int, default=4, help='number of cpu threads to use during batch generation')  # Reduced num_workers
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        # Set model input
        real_nsr = Variable(input_A.copy_(batch['A']))  # non-shadow region: input
        random_sr = Variable(input_B.copy_(batch['B']))  # random real shadow region
        mask = Variable(input_C.copy_(batch['C']))       # non-shadow region mask
        real_ns = Variable(input_D.copy_(batch['D']))    # without real shadow region
        real_nsrs = Variable(input_E.copy_(batch['E']))  # without non-shadow region and real shadow region
        mask_dil = Variable(input_F.copy_(batch['F']))   # dilated mask
        ...
        if (i + 1) % opt.iter_loss == 0:
            log = f'Epoch: {epoch}, [iter {curr_iter}], [loss_G {loss_G:.5f}], [loss_G_identity {loss_identity_B:.5f}], [loss_G_GAN {loss_GAN_A2B:.5f}], [loss_D {loss_D_B:.5f}], [loss_shadow {loss_shadow:.5f}]'
            print(log)
    ...
    if epoch % opt.snapshot_epochs == 0:
        torch.save(netG_A2B.state_dict(), f'ckpt/netG_A2B_{epoch + 1}.pth')
        torch.save(netG_1.state_dict(), f'ckpt/netG_1_{epoch + 1}.pth')
        torch.save(netG_2.state_dict(), f'ckpt/netG_2_{epoch + 1}.pth')
        torch.save(netD_B.state_dict(), f'ckpt/netD_B_{epoch + 1}.pth')
