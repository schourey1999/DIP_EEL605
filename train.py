...
# Networks
netG_A2B = Generator_S2F().to(device)  # shadow to shadow_free
netD_B = Discriminator().to(device)
netG_1 = Generator_S2F().to(device)  # shadow to shadow_free
netG_2 = Generator_F2S().to(device)  # shadow to shadow_free
...
# Losses
criterion_GAN = torch.nn.MSELoss().to(device)  # lsgan
...
input_A = torch.zeros((opt.batchSize, 3, opt.size, opt.size), device=device)
input_B = torch.zeros((opt.batchSize, 3, opt.size, opt.size), device=device)
input_C = torch.zeros((opt.batchSize, 1, opt.size, opt.size), device=device)
input_D = torch.zeros((opt.batchSize, 3, opt.size, opt.size), device=device)
input_E = torch.zeros((opt.batchSize, 3, opt.size, opt.size), device=device)
input_F = torch.zeros((opt.batchSize, 1, opt.size, opt.size), device=device)
target_real = torch.ones((opt.batchSize,), device=device, requires_grad=False)
target_fake = torch.zeros((opt.batchSize,), device=device, requires_grad=False)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        loss_shadow = criterion_cycle(
            torch.cat(((output[:, 0] + 1.0) * mask_dil - 1.0, output[:, 1:] * mask_dil), 1),
        ...
        if (i + 1) % opt.iter_loss == 0:
            ...
            avg_log = '[the last %d iters], [loss_G %.5f], [D_B_losses %.5f],' % (opt.iter_loss, G_losses[-1], D_B_losses[-1])
            print(avg_log)
